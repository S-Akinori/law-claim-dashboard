...
import { MasterQuestionRouteDialog } from "@/components/master-question-route-dialog"
...
interface Condition {
  ...
  value: string | null
  condition_group: string
  required_master_question?: Question
  required_master_option?: Option
}
...
export default function RoutesPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      // 全ての選択肢を取得
      const { data: optionsData, error: optionsError } = await supabase.from("master_options").select("*")
      ...
      optionsData?.forEach((option) => {
        if (!optionsByQuestionId[option.master_question_id]) {
          optionsByQuestionId[option.master_question_id] = []
        }
        optionsByQuestionId[option.master_question_id].push(option)
      })
      ...
      const { data: routesData, error: routesError } = await supabase
        ...
        .select(`
          *,
          from_masetr_question:from_master_question_id(*),
          next_master_question:next_master_question_id(*)
        `)
        .order("created_at", { ascending: true })
      ...
      const routesWithConditions = await Promise.all(
        (routesData || []).map(async (route) => {
          const { data: conditionsData, error: conditionsError } = await supabase
            ...
            .select(`
              *,
              required_master_question:required_master_question_id(*),
              required_master_option:required_master_option_id(*)
            `)
            .eq("master_question_id", route.from_master_question_id)
    ...
    try {
      ...
      if (targetRoute.conditions && targetRoute.conditions.length > 0) {
        const { error: conditionsError } = await supabase
          .from("master_conditions")
          .delete()
          .eq("master_question_id", targetRoute.from_master_question_id)
      ...
      // ルートを削除
      const { error: routeError } = await supabase.from("master_question_routes").delete().eq("id", routeId)
  ...
  const getConditionDescription = (condition: Condition): string => {
    let description = `「${condition.required_master_question?.title || "不明な質問"}」が `

    if (condition.required_master_option_id) {
      description += `「${condition.required_master_option?.text || "不明な選択肢"}」を選択した場合`
    } else {
            ...
            <CardContent>
              <Accordion type="multiple" className="w-full">
                {questions.map((question) => {
                  // この質問からの遷移ルートを取得
                  const questionRoutes = routes.filter((route) => route.from_master_question_id === question.id)
                              ...
                              <div className="flex items-center justify-between">
                                <div className="flex-1">
                                  <div className="flex items-center mb-2">
                                    <div className="font-medium">{getQuestionTitle(route.from_master_question_id)}</div>
                                    <ArrowRight className="mx-2 h-4 w-4" />
                                    <div className="font-medium">{getQuestionTitle(route.next_master_question_id)}</div>
                                  </div>
