...
export default function EditQuestionPage() {
  ...
  const fetchQuestion = async () => {
    try {
      ...
      const { data: optionsData, error: optionsError } = await supabase
        .from("master_options")
        .select("*")
        .eq("master_question_id", questionId)
  ...
  const handleImageUpload = async (index: number, file: File) => {
    ...
    try {
      ...
      const accountId = "32e26c1d-cd9d-4b0c-8c45-d8077ee04803"
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      for (const option of optionsToDelete) {
        const { error: deleteOptionError } = await supabase.from("master_options").delete().eq("id", option.id)
      ...
      if (optionsToAdd.length > 0) {
        ...
        const { error: addOptionsError } = await supabase.from("master_options").insert(optionsToInsert)
