...
export default function NewQuestionPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data: accountData, error: accountError } = await supabase.from("accounts").select("id").eq("user_id", user.id).single()
      ...
      if (options.length > 0) {
        const optionsToInsert = options.map((option) => ({
          master_question_id: questionData[0].id,
          text: option.text,
          image_url: option.image_url || null,
        }))

        const { error: optionsError } = await supabase.from("master_options").insert(optionsToInsert)
