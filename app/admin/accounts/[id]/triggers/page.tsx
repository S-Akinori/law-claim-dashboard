...
export default async function TriggersPage({ params }: TriggerPageProps) {
  const { id: accountId } = params
  const supabase = await createClient()
  const { data: triggersData, error: triggersError } = await supabase.from("start_triggers").select(`*, questions(*)`).eq("account_id", accountId)
    ...
    <>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <Button asChild>
            <Link href={`/admin/accounts/${accountId}/triggers/new`} className="flex items-center">
          ...
          <CardContent>
            {triggersData.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-6 text-center">
                ...
                <Button asChild>
                  <Link href={`/admin/accounts/${accountId}/triggers/new`}>
                    <Plus className="mr-2 h-4 w-4" />
                    新規トリガー
                  </Link>
                </Button>
                ...
                <TableBody>
                  {triggersData.map((trigger) => (
                    <TableRow key={trigger.id}>
                      ...
                      <TableCell>{trigger.questions.title}</TableCell>
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            <DropdownMenuItem className="text-destructive focus:text-destructive" asChild>
                              <Link href={`/admin/accounts/${trigger.account_id}/triggers/${trigger.id}`}>
