...
export function QuestionRouteDialog({
  ...
  useEffect(() => {
    if (open) {
      if (route) {
        // 編集モード
        setIsEdit(true)
        setFromQuestionId(route.from_question_id)
        setNextQuestionId(route.next_question_id)
  ...
  const handleConditionChange = (
    index: number,
    field: "required_question_id" | "required_option_id" | "operator" | "value",
    value: string | null,
  ) => {
    const newConditions = [...conditions]
    ...
    if (field === "required_question_id") {
      newConditions[index].required_option_id = null
    ...
    try {
      ...
      if (isEdit && route) {
        ...
        for (const condition of conditionsToDelete) {
          const { error: deleteError } = await supabase.from("conditions").delete().eq("id", condition.id)
        ...
        const validConditions = conditions.filter(
          (condition) => condition.required_question_id && (condition.required_option_id || condition.operator),
        )
        ...
        if (validConditions.length > 0) {
          ...
          const { error: insertConditionError } = await supabase.from("conditions").insert(conditionsToInsert)
                        ...
                        <div key={condition.id || index} className="space-y-3 border p-4 rounded-md">
                          ...
                          <div className="space-y-3">
                            <div>
                              ...
                              <Select
                                value={condition.required_question_id}
                                onValueChange={(value) => handleConditionChange(index, "required_question_id", value)}
                              >
                            ...
                            {condition.required_question_id && (
                              <>
                                {getQuestionType(condition.required_question_id) === "button" ||
                                getQuestionType(condition.required_question_id) === "image_carousel" ? (
                                  // ボタンや画像カルーセルの場合は選択肢から選ぶ
                                  <div>
                                    <Label htmlFor={`requiredOption-${index}`}>選択肢</Label>
                                    <Select
                                      value={condition.required_option_id || undefined}
                                      onValueChange={(value) =>
                                        handleConditionChange(index, "required_option_id", value)
                                      }
                                    >
                                      ...
                                      <SelectContent>
                                        {options[condition.required_question_id]?.map((option) => (
